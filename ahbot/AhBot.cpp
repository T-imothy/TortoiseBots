...
{
    Config config;
    std::string configFile = sConfig.GetStringDefault("AhBot.ConfigFile", "");
    if (configFile.empty())
    {
        std::string const mainConfig = sConfig.GetFilename();
        size_t const slash = mainConfig.find_last_of("/\\");
        configFile = slash == std::string::npos ? SYSCONFDIR "ahbot.conf"
            : mainConfig.substr(0, slash + 1) + "ahbot.conf";
    }
    if (!config.SetSource(configFile.c_str(), "AHBot_"))
        return false;
